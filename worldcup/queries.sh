...
echo "$($PSQL "SELECT name FROM games RIGHT JOIN teams ON games.winner_id = teams.team_id WHERE games.round = 'Final' AND games.year = '2018'")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games ON teams.team_id = games.opponent_id OR teams.team_id = games.winner_id WHERE round = 'Eighth-Final' AND year = '2014' ORDER BY name ASC")"
